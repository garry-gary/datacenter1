...
{   
    ...
    if(argc!=4){
        ...
        printf("本程序把pathname目录及子目录中timeout天之前匹配matchstr文件全部压缩，timeout可以是小数。\n");
    ...
    while(true){
        ...
        if(system(strCmd)==0) printf("gzip %s ok.\n",Dir.m_FullFileName);
        else printf("gzip %s failed.\n",Dir.m_FullFileName);
        }
    }   
    return 0;   
}
